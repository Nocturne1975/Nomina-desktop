...
class NominaWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.addFragmentButton = QtWidgets.QPushButton("Ajouter un fragment")
        self.addFragmentButton.clicked.connect(self.ajouter_fragment)
        button_layout.addWidget(self.addFragmentButton)
...
def ajouter_fragment(self):
    from db import get_all_cultures, get_all_categories, insert_fragment
    dialog = QtWidgets.QDialog(self)
    dialog.setWindowTitle("Ajouter un fragment d'histoire")
    layout = QtWidgets.QFormLayout(dialog)
    texte = QtWidgets.QLineEdit()
    appliesTo = QtWidgets.QLineEdit()
    genre = QtWidgets.QLineEdit()
    minNameLength = QtWidgets.QSpinBox()
    minNameLength.setMinimum(0)
    minNameLength.setMaximum(1000)
    maxNameLength = QtWidgets.QSpinBox()
    maxNameLength.setMinimum(0)
    maxNameLength.setMaximum(1000)
    ...
    layout.addRow("Applies To", appliesTo)
    layout.addRow("Genre", genre)
    layout.addRow("Min Name Length", minNameLength)
    layout.addRow("Max Name Length", maxNameLength)
    layout.addRow("Culture", culture_combo)
    layout.addRow("Catégorie", categorie_combo)
    btn = QtWidgets.QPushButton("Ajouter")
    layout.addWidget(btn)
    btn.clicked.connect(lambda: self._insert_fragment(
        dialog, texte.text(), appliesTo.text(), genre.text(),
        minNameLength.value(), maxNameLength.value(),
        culture_combo.currentData(), categorie_combo.currentData()
    ))
    dialog.exec()

def _insert_fragment(self, dialog, texte, appliesTo, genre, minNameLength, maxNameLength, culture_id, categorie_id):
    from db import insert_fragment
    insert_fragment(texte, appliesTo, genre, minNameLength, maxNameLength, culture_id, categorie_id)
    self.load_fragments()
    dialog.accept()

def modifier_fragment(self):
    from db import get_all_cultures, get_all_categories, update_fragment
    ...
    old_minNameLength = int(self.fragmentsTable.item(row, 4).text()) if self.fragmentsTable.item(row, 4).text().isdigit() else 0
    old_maxNameLength = int(self.fragmentsTable.item(row, 5).text()) if self.fragmentsTable.item(row, 5).text().isdigit() else 0
    ...
    minNameLength.setMinimum(0)
    minNameLength.setMaximum(1000)
    minNameLength.setValue(old_minNameLength)
    maxNameLength = QtWidgets.QSpinBox()
    maxNameLength.setMinimum(0)
    maxNameLength.setMaximum(1000)
    maxNameLength.setValue(old_maxNameLength)
    culture_combo = QtWidgets.QComboBox()
    categorie_combo = QtWidgets.QComboBox()
    culture_combo.addItem("Aucune", None)
    for c in get_all_cultures():
    ...
    layout.addWidget(btn)
    btn.clicked.connect(lambda: self._update_fragment(
...
def _update_fragment(self, dialog, fragment_id, texte, appliesTo, genre, minNameLength, maxNameLength, culture_id, categorie_id):
